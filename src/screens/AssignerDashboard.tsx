...
type Props = {
  profile: UserProfile
  onLogout: () => void
  isTemporaryAssigner?: boolean
}

export function AssignerDashboard({
  profile,
  onLogout,
  isTemporaryAssigner = false,
}: Props) {
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-sm font-semibold text-purple-700">
                  Temporary Assigner Access Active
                </p>

                <h2 className="mt-1 text-xl font-bold text-slate-900">
                  You are currently acting as an assigner
                </h2>

                <p className="mt-2 text-sm text-slate-600">
                  Your original role is still Engineer. This access is temporary and can be used for case assignment until it expires.
                </p>
              </div>

              <span className="rounded-full bg-white px-4 py-2 text-xs font-semibold text-purple-700 ring-1 ring-purple-200">
                Temporary Access
              </span>
            </div>
          </section>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {statusOptions.map((option) => (
                        <button
                          ...
                          className={`rounded-xl px-4 py-2 text-sm font-medium transition disabled:opacity-60 ${selectedEngineer.status === option.value
